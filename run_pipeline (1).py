...
class Monitor:
    def __init__(self, root: Path, reprocess_existing: bool = False):
        ...
        self.overrides_path = self.data_dir / "manual_overrides.json"
        ...
        overrides_raw = load_json(self.overrides_path, {})
        self.overrides: dict[str, Any] = overrides_raw if isinstance(overrides_raw, dict) else {}
    ...
    def apply_manual_overrides(self) -> int:
        """Re-apply stance corrections exported from the dashboard's 'تصحيح التصنيف'
        button. Keys look like '<article_id>::<target_id>' and map to
        {"label": "favorable|critical|neutral|mixed|unclear", ...}. Because
        merge_articles() overwrites stance fields whenever an article is
        re-scraped, this must run *after* merge_articles()/assign_events()
        on every run so manual corrections survive future collection passes.
        """
        ...
        label_ar = {
            "favorable": "داعم",
            "critical": "ناقد",
            "neutral": "محايد أو إخباري",
            "mixed": "مختلط",
            "unclear": "غير واضح",
        }
        applied = 0
        by_id = {article.get("id"): article for article in self.news if article.get("id")}
        for key, correction in self.overrides.items():
            if "::" not in key:
                continue
            article_id, target_id = key.split("::", 1)
            article = by_id.get(article_id)
            if not article:
                continue
            label = correction.get("label")
            if label not in label_ar:
                continue
            for stance in article.get("stances", []):
                if stance.get("target_id") != target_id:
                    continue
                if "manual_override" not in stance:
                    stance["manual_override_original_label"] = stance.get("label")
                stance["label"] = label
                stance["label_ar"] = label_ar[label]
                stance["manual_override"] = True
                applied += 1
        return applied
    ...
    def run(self) -> dict[str, Any]:
        ...
        self.assign_events()
        overrides_applied = self.apply_manual_overrides()
        if overrides_applied:
            print(f"✎ أُعيد تطبيق {overrides_applied} تصحيحاً يدوياً محفوظاً في data/manual_overrides.json")
